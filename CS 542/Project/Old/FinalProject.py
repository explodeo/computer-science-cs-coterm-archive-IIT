...
def readMatrix(ma):
	...
	while(not done):
		...
		Label(rdir, text="Matrix file must be delimited in tabs, spaces, or commas.").grid(row=0)
		#Label(rdir, text=".").grid(row=x)
		rdir.file =  filedialog.askopenfilename(initialdir = "/",title = "Select Matrix Topology File",filetypes = (("Text Document","*.txt"),("Comma-Seprated Values",".csv"),("Tab-Separated Values",".tsv")))
		if rdir.file:
			done = True
			fname = str(rdir.file)
			rdir.destroy()
...
def findAllPaths(graph,s,d): #find all paths with a depth up to matrix-1 number of nodes
	...
	def getAllPaths(src, dest, v, path, G): #Depth-First-Search
		...
		if src == dest:
			...
			for x in G[src]:
				if not v[x]:
					getAllPaths(x,dest,v,path, G)#depth+1)
		v[src] = False
		path.pop()

...
public class Simulator {
	...
	public static void main(String[] args) throws IOException { //will break if file not found
		...
		File inFile; boolean filenotfound = true;
		while(filenotfound) {
			try {
		...
		while(!done){
			time++; //starts time at 0 that's why it is incremented first
			for (index = 0; index<=MAX_LINE_INDEX; index++) { //checks and moves vehicles 
				if (tollBooths[index].getLineLength()>0) { //handles empty tollboothlines
					...
					if (time-tollBooths[index].getQueue(0).getFrontTime() >= tollBooths[index].getQueue(0).getPassingTime()) { //if equal or exceeds the needed value to pass line
			...
			for (index = count; index<vehicles.length; index++ ) {	//adds vehicles to shortest lines when time=VehicleInTime
				if (vehicles[index].getTimeIn()<=time) { 
					for (TollBoothLine line : tollBooths) { 
						//if (line.getLineLength()>8 && line.getLineType().equalsIgnoreCase("A")){ System.out.println("TIME THIS BREAKS: "+time); }
					...
					if (okToAdd) { //will only add a vehicle if a line is not full. flag var later is reset.
		...
		//FInal Stats fro program are prnted below
		int linecount = 0;
		...
		System.out.println("Max Manual Wait = " + doneList.getMaxManualWait()); System.out.println("Max Automatic Wait = " + (tollBooths[MAX_LINE_INDEX].getMaxLengthReached()-1)); //because the automatic vehicles have a wait of 1 second, this minimizes the use of resources via less comparisons than the other method
		System.out.println("Avg Manual Wait = " + doneList.getAverageManualWait()); System.out.println("Avg Auto Wait = " + doneList.getAverageAutomaticWait());
	...
	public static Vehicle[] fileToArray(File file) throws IOException { //errors will be thrown if parameters in file are incorrect.
		...
		scan.close(); return arr; //prevents resource leaks
	}

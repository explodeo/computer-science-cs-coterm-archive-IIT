...
int init = 0; //flag for if syscall is initialized
struct spinlock lock; //lock for init
...
syscall(void)
{
  ...
  if (!init){
    initlock(&lock, "getcount");
    memset(curproc->syscounts, 0, sizeof curproc->syscounts); //zero fill syscounts
    init = 1;
  }
  
  acquire(&lock);
  curproc->syscounts[num]++; //update count on syscall
  release(&lock);
